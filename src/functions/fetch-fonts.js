...
export async function proxyStylesheet(url, request) {
...
export async function processRequest(request, event) {
      ...
      if (response && response.status === 200) {
        ...
        try {
          if (cache) {
            ...
            self.waitUntil(cache.put(cacheKeyRequest, cacheResponse));
          };
        } catch (e) {
